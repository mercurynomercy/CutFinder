"""OpenAI-compatible server health-check utility.

Extracted from the Makefile ``check-openai`` target so it can be unit-tested
with fake HTTP responses.

Usage from CLI::

    python -m cutfinder.adapters.openai_check
"""
...
def check_openai_compat(
    ...
    """Probe the OpenAI-compatible server and return a sorted list of model IDs.
    ...
    Examples
    --------
    >>> check_openai_compat("http://localhost:8000/v1", "test-key")  # doctest: +SKIP
    ['model-1', 'Qwen3.6-35B-A3B']
    """

    if not api_key:
        print(
            "OPENAI_API_KEY is empty — set it in the Settings UI or as an env var",
            file=_sys.stderr,
        )
        _sys.exit(1)
    ...
    try:
        ...
        raise RuntimeError(f"connection to the OpenAI-compatible server failed ({e})") from e

    if resp.status_code != 200:
        raise RuntimeError(
            f"OpenAI-compatible server returned HTTP {resp.status_code}: {resp.text}"
        )

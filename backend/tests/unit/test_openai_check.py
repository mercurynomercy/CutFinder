"""Unit tests for cutfinder.adapters.openai_check.

Uses httpx ``MockTransport`` to fake the OpenAI-compatible server's /models
endpoint without requiring a running server.
"""
...
def _call_check(models, expected_models=None, status_code=200):
    import cutfinder.adapters.openai_check as mod
    ...
    with mock.patch.object(mod.httpx, "get", side_effect=mock_get):  # type: ignore[attr-defined]
        if expected_models is not None:
            return mod.check_openai_compat(
...
class TestCheckOpenAICompatOk:
    ...
    def test_with_expected_models_present(self):
        ...
        with mock.patch.object(mod.httpx, "get", side_effect=mock_get):  # type: ignore[attr-defined]
            result = mod.check_openai_compat(
                base_url="http://localhost:8000/v1",
                api_key="test-key",
    ...
    def test_exits_with_error_message(self, stderr_capture):
        ...
        with mock.patch.object(mod.httpx, "get"):  # type: ignore[attr-defined]
            with pytest.raises(SystemExit) as exc_info:
                mod.check_openai_compat(base_url="http://localhost:8000/v1", api_key="")
...
class TestHttpErrors:
    ...
    def test_connection_error(self):
        ...
        with mock.patch.object(mod.httpx, "get", side_effect=mock_get):  # type: ignore[attr-defined]
            with pytest.raises(RuntimeError) as exc_info:
                mod.check_openai_compat(base_url="http://localhost:8000/v1", api_key="test-key")
        assert "connection" in str(exc_info.value).lower() and "failed" in str(exc_info.value).lower()

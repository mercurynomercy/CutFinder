# CutFinder Design System

> AI 驱动的短视频素材管理与初剪工具 — 完整 UI 页面清单与设计规范

---

## Pages

| # | Page | File | Route | Description |
|---|------|------|-------|-------------|
| 0 | **Launcher** | `index.html` | `/` | 启动页 — 4 张入口卡片 |
| 1 | **素材库** | `home.html` | `/home` | 网格画廊 — 按日期分组，左侧筛选面板 |
| 2 | **设置** | `settings.html` | `/settings` | 双栏设置页 — OMLX、语音引擎、处理选项 |
| 3 | **任务队列** | `tasks.html` | `/tasks` | 任务表格 — 扫描/关键帧/初剪状态 |
| 4 | **初剪** | `rough-cut.html` | `/rough-cut` | 三栏布局 — 对话侧栏 + 聊天 + 分镜表 |
| 5 | **片段详情** | `clip-detail.html` | `/clip-detail` | 右抽屉 — 标签、建议剪辑、转录、元数据 |
| 6 | **初剪设置** | `rough-cut-settings.html` | `/rough-cut-settings` | 生成选项 + 导演提示词编辑器 |
| 7 | **字幕导出** | `subtitles.html` | `/subtitles` | 选择视频 + 输出文件夹 + 格式选项（iTT/SRT） |
| 8 | **日志** | `logs.html` | `/logs` | 后端日志查看器 — 级别筛选 + 清空，从设置页「系统工具」进入 |

---

## Navigation Map

```
index.html (Launcher)
  ├── home.html (素材库)
  │     └── clip-detail.html (片段详情) ← 点击任意视频卡片打开
  ├── settings.html (设置)
  ├── tasks.html (任务队列)
  ├── rough-cut.html (初剪)
  │     └── rough-cut-settings.html (初剪设置) ← 点击"初剪设置"按钮打开
  └── subtitles.html (字幕导出) ← 独立功能，从启动页直接进入

settings.html (设置)
  └── logs.html (日志) ← 点击"系统工具"中的"查看日志"打开
```

---

## Design Tokens

### Colors

| Token | Light | Dark | Usage |
|-------|-------|------|-------|
| `--bg-canvas` | `#EEF0F3` | `#0E0F11` | 页面背景 |
| `--surface-1` | `#FFFFFF` | `#16181B` | 卡片/面板背景 |
| `--surface-2` | `#F6F7F9` | `#1E2125` | 输入框/次级背景 |
| `--surface-3` | `#E4E7EC` | `#282C31` | 悬停态背景 |
| `--border` | `#D8DCE2` | `#2E333A` | 边框 |
| `--border-strong` | `#BCC2CB` | `#3A4048` | 强边框 |
| `--text-primary` | `#1A1D21` | `#F2F4F7` | 主文本 |
| `--text-secondary` | `#4B5563` | `#A4ACB9` | 次级文本 |
| `--text-muted` | `#6B7280` | `#6B7280` | 弱化文本 |
| `--primary` | `#5256E0` | `#6366F1` | 主色（按钮/高亮） |
| `--primary-hover` | `#4146C4` | `#7077F2` | 主色悬停 |
| `--primary-press` | `#383DB0` | `#525AE0` | 主色按下态 |
| `--primary-soft` | `rgba(82,86,224,0.12)` | `rgba(99,102,241,0.15)` | 主色柔光背景 |
| `--roll-a` | `#B45309` | `#F59E0B` | A-roll 标识 |
| `--roll-b` | `#0F766E` | `#2DD4BF` | B-roll 标识 |
| `--roll-photo` | `#BE185D` | `#F472B6` | 照片标识 |
| `--success` | `#15803D` | `#22C55E` | 成功/完成 |
| `--success-bg` | `rgba(21,128,61,0.1)` | `rgba(34,197,94,0.12)` | 成功状态背景 |
| `--warning` | `#B45309` | `#F59E0B` | 警告 |
| `--error` | `#DC2626` | `#EF4444` | 错误 |
| `--processing` | `#5256E0` | `#6366F1` | 处理中 |

### Typography

| Token | Value | Usage |
|-------|-------|-------|
| `--font-ui` | `"Inter", "PingFang SC", -apple-system, system-ui, sans-serif` | 界面文本 |
| `--font-mono` | `"JetBrains Mono", ui-monospace, "SF Mono", monospace` | 代码/时间戳/文件大小 |
| `--text-xs` | `12px` | 标签、辅助文本 |
| `--text-sm` | `13px` | 按钮、输入框、列表项 |
| `--text-base` | `14px` | 正文 |
| `--text-md` | `15px` | 小标题 |
| `--text-lg` | `16px` | 页面标题 |
| `--text-xl` | `20px` | 大标题 |
| `--text-2xl` | `24px` | 页面主标题 |
| `--text-3xl` | `32px` | Launcher 标题 |

### Spacing & Radius

| Token | Value |
|-------|-------|
| `--radius-sm` | `6px` |
| `--radius-md` | `8px` (10px on launcher) |
| `--radius-lg` | `10px` (14px on launcher) |
| `--shadow-1` | `0 1px 2px rgba(0,0,0,0.06)` |
| `--shadow-2` | `0 4px 12px rgba(0,0,0,0.08)` |

---

## Page Details

### 0. Launcher (`index.html`)

**Layout:** 居中卡片网格，2×2

**Components:**
- Logo + 标题 "CutFinder"
- 副标题 "AI 驱动的短视频素材管理与初剪工具"
- 5 张入口卡片（素材库 / 设置 / 任务队列 / 初剪 / 字幕导出）
- 右上角主题切换按钮

**Card structure:**
- 图标（带背景色）
- 标题
- 描述
- "打开 →" 链接

---

### 1. 素材库 (`home.html`)

**Layout:** 顶部栏 + 左侧筛选面板 + 右侧画廊

**Top Bar:**
- Logo + "CutFinder"
- 搜索框 "搜索片段…"
- 导航：任务 / 初剪
- 分隔线
- 设置图标 / 主题切换
- "扫描" 主按钮

**Sidebar (240px):**
- 标题 "筛选" + 收起按钮
- 搜索框 "搜索片段…"
- **类型筛选:** 全部 / A-roll / B-roll / 照片（tab 切换）
- **日期筛选:** 可展开的日期列表，显示片段数
- **标签筛选:** 标签芯片网格 + "显示全部 290"

**Gallery:**
- 工具栏："展开/收起筛选" / "47 个片段" / **"全部展开／全部收起"按钮**（折叠所有日期分组）/ 排序下拉
- 按日期分组（`2016/08/31 · 45 个片段`），分组标题可点击折叠/展开，悬停显示"在 Finder 中显示"图标
- 视频卡片网格（`auto-fill, minmax(220px, 1fr)`）

**Video Card:**
- 缩略图（16:9，渐变占位符）
- 类型徽章（A-roll / B-roll / 照片，毛玻璃背景）
- 时长徽章（右下角，黑色半透明）
- **悬停显示"在 Finder 中显示"图标按钮（右上角，毛玻璃背景）**
- 文件名
- 描述（单行截断）
- 标签芯片 + "+N" 更多
- **可点击 → 打开 clip-detail.html**

**Status Bar (底部固定):**
- 处理进度 "▸ 处理中 18/40"
- 当前文件名（等宽字体）
- 状态 "转写中…"
- 进度条 + 百分比
- **另有独立的全局 Ambient JobsPanel（跨页面浮层，见 Component Library）承载后台任务的进度条 + Toast 通知，与本状态栏是两套并行机制**

---

### 2. 设置 (`settings.html`)

**Layout:** 顶部栏 + 双栏设置页 + 底部固定保存栏

**Two Columns:**

**Left Column:**
- **界面语言:** 下拉选择（中文 / English）
- **素材文件夹:** 路径输入 + 移除按钮 + "添加文件夹"
- **素材库路径:** 路径输入 + "选择…" 文件夹按钮 → **切换库前弹出 ConfirmDialog 显示目标路径，确认后才生效**
- **OMLX 连接:**
  - 连接状态指示（绿色圆点 "已连接"）+ **"测试连接"按钮**（testing/已连接/失败三态）
  - Base URL（等宽输入）
  - API 密钥（密码输入，显示"已配置"）
  - 文本模型（等宽输入，默认 Qwen3.6-35B-A3B）
  - 视觉模型（等宽输入，默认 Qwen3-VL-8B）
  - **字段校验失败时在输入框下方显示内联错误提示（红色边框 + 错误文案）**
- **语音引擎:**
  - 引擎选择（Whisper / Qwen3-ASR + ForcedAligner）
  - Qwen ASR 模型（等宽输入）
  - ForcedAligner 模型（等宽输入）
  - 分段最大秒数（数字输入，默认 120）

**Right Column:**
- **处理选项:**
  - 视频扩展名芯片（.mov / .mp4 / .m4v）+ 添加
  - 照片扩展名芯片（.jpg / .jpeg / .png / .heic）+ 添加
- **B-roll 帧数:** 数字输入（默认 5）
- **VAD 阈值 (0–1):** 数字输入（默认 0.35）
- **A-roll 转写前分离人声:** Toggle（默认开启）
- **AI 输出语言:** 下拉选择（中文 / English）
- **每段关键帧建议数:** 数字输入（默认 3，范围 1–10）
- **扫描后自动推荐关键帧:** Toggle（默认开启）

**System Tools (系统工具):**
- "建议全部关键帧" / "清理已删除文件"（弹出 ConfirmDialog 确认）/ "查看日志" → `logs.html`
- 各操作完成后通过 Toast 通知反馈结果

**Save Bar (底部固定):**
- "保存设置" 按钮（右对齐）

---

### 3. 任务队列 (`tasks.html`)

**Layout:** 顶部栏 + 全局暂停提示条（条件显示）+ 表格页面

**Top Bar Actions:**
- "暂停" 按钮（带暂停图标）
- 关闭按钮

**Queue Banner（队列整体暂停时显示）:**
- 暂停图标 + "处理队列已暂停" + "继续处理" 按钮（点击弹出 ConfirmDialog 二次确认）

**Table:**
| 列 | 宽度 | 内容 |
|---|------|------|
| ID | 60px | `#7` 等宽字体 |
| 类型 | 80px | 徽章：扫描（蓝色）/ 关键帧（橙色）/ 初剪（青色）/ **重新分析（粉色）/ 字幕（灰色）** |
| 状态 | 100px | 已完成（绿色）/ 处理中（蓝色）/ 等待中（灰色）/ 失败（红色，**失败项数 + 可截断错误提示 tooltip**）/ **已暂停（橙色）** |
| 进度 | 140px+ | 进度条 + `1/1` 等宽标签 |
| 开始时间 | 160px | `2026-08-02 15:24:08` 等宽字体 |
| 操作 | 180px | "删除" 按钮（红色边框）+ **失败任务显示"重试失败"、已暂停任务显示"继续"（reanalyze 类型除外，不可恢复）** |

---

### 4. 初剪 (`rough-cut.html`)

**Layout:** 顶部栏 + 三栏布局

**Column 1: 对话侧栏 (220px)**
- "新建对话" 按钮
- 收起按钮
- 对话列表（活动高亮 / 未命名灰色斜体），**每项悬停显示删除图标 → 点击弹出 ConfirmDialog 二次确认后删除**

**Column 2: 聊天面板 (420px)**
- 消息列表（用户消息右对齐蓝色 / 系统消息左对齐白色边框）
- 聊天输入区：
  - 自适应高度文本框
  - "初剪设置" 按钮 → 打开 `rough-cut-settings.html`
  - "发送" 按钮（禁用态/启用态）

**Column 3: 分镜表（弹性宽度）**
- 标题 "分镜表" + **全屏切换按钮（隐藏侧栏 + 聊天面板，分镜表独占宽度）** + "复制为 Markdown"
- 按日期分组
- 分镜条目卡片：
  - 序号 + A/B-roll 类型圆点
  - 缩略图（120×68）+ 文件名
  - 时间戳 `[A-roll] 描述`
  - 笔记
  - 文件名

**生成过程面板 (chat-msg-inprogress，生成中显示于聊天列表):**
- 标题行："生成过程（N 步）" + 取消按钮
- 常驻当前步骤行：**多天生成时显示"第 X/Y 天"徽标** + 当前步骤文案 + 实时耗时计数器
- 可折叠的步骤列表（完成态 ✓ / 加载中 spinner）
- 进度条 + `已完成步数/总步数`
- 顶部栏同步显示生成进度圆环 + 百分比，浏览器标签页标题同步显示 `(index/total)`

**待澄清问题状态:**
- 系统消息 + 选项按钮组（如日期范围、时长、风格、画幅），点击选项即继续生成

---

### 5. 片段详情 (`clip-detail.html`)

**Layout:** 顶部栏 + 左预览 + 右抽屉 + 底部栏

**Left: Video Preview**
- 视频预览区（黑色背景，渐变占位符）
- 文件名（等宽）+ **"在 Finder 中显示"图标按钮**
- 描述
- 标签芯片
- 日期（右对齐）+ **日期来源标识（当日期由文件创建时间推断而非元数据时，显示"从文件推断的拍摄日期"提示图标）**

**Right: Detail Drawer (480px)**
- **Tags:**
  - 标签芯片网格（带 × 移除按钮）
  - "Add tag..." 输入框 + "Add" 按钮
- **Suggested Cuts:**
  - "Suggest keyframes" 按钮
  - 建议剪辑卡片（缩略图 + 时间 + 描述），**可点击 → 在左侧预览中定位到该片段**
- **Transcript** (可折叠):
  - 完整转录文本
  - 时间轴分段（`1.2s` / `5.1s` ...）
- **Source File** (可折叠):
  - 文件路径（等宽字体）
- **Metadata** (可折叠):
  - Duration / Resolution / Frame rate / Codec 网格

**Bottom Bar:**
- A-roll / B-roll 切换按钮组
- "Re-analyze" 按钮

---

### 6. 初剪设置 (`rough-cut-settings.html`)

**Layout:** 顶部栏 + 居中设置卡片 + 底部操作栏

**Card Header:**
- "Rough-cut settings" 标题
- "Custom prompt in use" 链接

**Generation Options:**
- **Generation mode:** 下拉（Agent (deeper) / Fast）
  - 说明文本
- **Max tool rounds (agent):** 数字输入（默认 24）
  - 说明文本
- **Per-day catalog size (agent, tokens):** 数字输入（默认 50000）
  - 说明文本
- **Critic review pass:** Checkbox
  - 说明文本
- **Vision look-ups per generation:** 数字输入（默认 6）
  - 说明文本
- **Per-day catalog size (fast, tokens):** 数字输入（默认 40000）
  - 说明文本

**Director Prompt:**
- 说明文本（占位符说明）
- 多行文本编辑器（默认中文导演提示词）

**Footer:**
- "Reset to default" 按钮
- "Cancel" 按钮
- "Save" 按钮（主色）

---

### 7. 字幕导出 (`subtitles.html`)

**Layout:** 顶部栏 + 居中卡片列表 + 底部操作栏

**Top Bar:**
- "字幕导出" 标题
- 主题切换按钮
- 关闭按钮 → `index.html`

**Content (居中，max-width 640px):**
- 说明文本 "选择一个已完成的视频和一个输出文件夹，然后导出从音频中转录的字幕。"

**Section 1: 视频**
- 标签 "视频"
- 状态 "未选择视频"（斜体弱化）
- "选择视频" 按钮 → 展开视频列表（mock 数据：文件名 + 时长）
- 选中后显示文件名 + 大小/时长 + "更换视频" 按钮

**Section 2: 输出文件夹**
- 标签 "输出文件夹"
- 状态 "未选择文件夹"（斜体弱化）
- "选择输出文件夹" 按钮 → 模拟选择 `~/Videos/CutFinder/exports`
- 选中后显示路径 + "更换文件夹" 按钮

**Section 3: 格式**
- 标签 "格式"
- 复选框：iTT（默认勾选）/ SRT（默认勾选）
- 辅助文本 "字幕语言跟随「设置」中配置的 AI 输出语言。"
- 分隔线
- "每条字幕最短时长（秒）" 数字输入（默认 2，范围 0–30）
- 辅助文本解释用途

**Footer:**
- **语音模型下载提示（首次使用某引擎且模型未下载到本地时显示）：** 柔光背景提示条 + 下载进度条
- 左侧状态文本（就绪时显示格式信息）
- 右侧："取消" 按钮 + "导出" 按钮（未就绪时禁用）
- 导出后显示 "导出中…" → "已完成"（绿色）→ 3 秒后重置

---

### 8. 日志 (`logs.html`)

**Layout:** 顶部栏（返回 + 标题 + 主题切换）+ 居中页面（max-width 900px）

**Toolbar:**
- 级别筛选：全部 / INFO / WARN / ERROR / 扫描（tab 切换）
- "清空" 按钮（右对齐）

**Log Container:**
- 等宽字体日志条目：时间戳 + 级别徽章（色彩区分）+ 消息文本
- 无匹配日志时显示居中空状态文案

**Entry Point:** 设置页「系统工具」→「查看日志」

---

## Component Library

### Buttons

| Type | Style | Usage |
|------|-------|-------|
| Primary | `background: var(--primary); color: white` | 主要操作（扫描、保存、发送） |
| Secondary | `border: 1px solid var(--border); background: var(--surface-2)` | 次要操作 |
| Destructive | `border: 1px solid var(--error); color: var(--error)` | 删除操作 |
| Icon-only | `width: 32px; height: 32px` | 主题切换、关闭 |

### Inputs

| Type | Style |
|------|-------|
| Text | `height: 32px; background: var(--surface-2); border: 1px solid var(--border)` |
| Mono | `font-family: var(--font-mono); font-size: var(--text-xs)` |
| Select | Custom arrow, `padding-right: 28px` |
| Number | `width: 100px; text-align: center` |
| Password | `type="password"` |
| Textarea | `min-height: 180px; resize: vertical` |

### Badges / Chips

| Type | Background | Text |
|------|-----------|------|
| Primary | `var(--primary-soft)` | `var(--primary)` |
| Roll A | `var(--roll-a)` | `white` |
| Roll B | `var(--roll-b)` | `white` |
| Photo | `var(--roll-photo)` | `white` |
| Warning | `rgba(180,83,9,0.1)` | `var(--warning)` |
| Tag | `var(--surface-2); border: 1px solid var(--border)` | `var(--text-primary)` |

### Toggles

- Track: `40×22px`, rounded 11px
- Thumb: `16×16px`, white with shadow
- Active: `background: var(--primary)`

### Confirm Dialog

替代 `window.confirm()`（在 macOS 原生应用的 WKWebView 中被禁用）。用于所有具破坏性或状态切换性质的操作：暂停/继续队列、清理素材库、切换素材库、删除初剪对话。

- 半透明遮罩（`rgba(0,0,0,0.4)`）+ 居中卡片（`--radius-lg`，`--shadow-2`）
- 标题 + 说明文字，破坏性操作时可附带目标路径/详情的等宽文本块
- 操作按钮：次要"取消" + 主要确认（危险操作用 `--error` 背景，其余用 `--primary` 或 `--warning`）
- 支持 Esc 键关闭 + 点击遮罩关闭

### Toast 通知

右下角堆叠通知，用于后台任务生命周期事件和批量操作结果反馈（如清理完成、连接测试结果）。

- 卡片：`--surface-1` 背景 + 左侧 3px 色条（success / error / info）+ `--shadow-2`
- 5 秒后自动淡出消失
- 由全局 `JobsPanel`（跨页面浮层，不在任何单一 `ui-design/*.html` 中，而是叠加于所有页面之上）驱动，承载扫描/关键帧/初剪等后台任务的常驻进度条

---

## Responsive Breakpoints

| Breakpoint | Behavior |
|-----------|----------|
| `≥ 900px` | 设置页双栏 |
| `< 900px` | 设置页单栏 |
| `≥ 1024px` | 素材库侧栏 240px |
| `< 1024px` | 素材库侧栏 200px |
| `< 768px` | 素材库侧栏绝对定位覆盖 |
| `< 500px` | Launcher 卡片单列 |

---

## Interactive Behaviors

### Theme Toggle
- Persisted to `localStorage('cutfinder-theme')`
- Values: `'light'` / `'dark'`
- Sun icon shown in dark mode, moon icon in light mode

### Collapsible Sections
- Used in clip-detail.html: Transcript, Source File, Metadata
- Chevron rotates 90° when open
- Body toggled via `.hidden` class

### Tag Management
- Add: input + Enter key or "Add" button
- Remove: × button on each chip
- Dynamic re-binding of remove handlers for new tags

### Settings Persistence
- `rough-cut-settings.html` saves to `localStorage('cutfinder-settings')`
- JSON structure: `{ mode, maxRounds, agentCatalog, criticReview, visionLookups, fastCatalog, directorPrompt }`
- Reset clears localStorage and reloads

---

## File Sizes

| File | Size | Lines |
|------|------|-------|
| `index.html` | 12 KB | 326 |
| `home.html` | 55 KB | 1,292 |
| `settings.html` | 36 KB | 903 |
| `tasks.html` | 25 KB | 687 |
| `rough-cut.html` | 57 KB | 1,583 |
| `clip-detail.html` | 31 KB | 888 |
| `rough-cut-settings.html` | 20 KB | 570 |
| `subtitles.html` | 25 KB | 753 |
| `logs.html` | 11 KB | 315 |
| **Total** | **281 KB** | **7,317** |

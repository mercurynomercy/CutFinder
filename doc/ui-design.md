...
- **初剪逐次生成参数不在此页**：`cut_director_mode`（生成模式）、`cut_max_tool_rounds`（最大工具轮数）、`cut_critic_enabled`（审片复检）、`cut_vision_budget`（视觉确认次数）属于初剪导演的逐次生成参数，放在**初剪页**的「初剪设置」弹窗（与导演 Prompt 同处），不在全局设置里（`tasks/28`）。

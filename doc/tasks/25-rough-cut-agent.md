...
> **状态：beta（已上线，持续改进中）。** 本地 function-calling 不收敛、整片一次性 JSON 跑飞，已两次按真机表现迭代：
> 退化为分阶段受限调用，再细化为**按拍摄日期逐日生成**（每个拍摄日期单独一次结构化补全，代码拼接为按日期分章的 plan）。
> 另增：消息内自然语言参数解析、可编辑导演 Prompt（`~/.cutfinder/config.json`，可重置）、当天按 `capture_time` 时间线排序、
> 分镜表带拍摄日期列、会话自动命名。分镜质量与默认 prompt 仍在打磨。详见 `detailed-design.md` §3.15。

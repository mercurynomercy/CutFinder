...
- [x] 真机跑多日初剪：聊天区滚动显示"第 k/N 天 · 查看片段 #X …"；分镜表随每天完成增量出现，顶部横幅提示生成中；全部完成后横幅消失、状态清空。

---

## 后续增强：更丰富的进度 UI，看清导演"思考到哪一步"

当前只有一行替换式状态（"正在生成第 1/6 天…"）。用户希望**信息更多、能看到思考轨迹**。**已做大部分**：

**后端：发更多、更具体的进度事件**（`director.py` 的 `on_progress` / `_run_day` `on_step`）：
- [x] **检索阶段**："正在检索素材…" → "找到 N 个片段、共 M 天，开始生成…"。
- [x] **每天起始**带素材数："正在生成第 k/N 天（date）· 本天 X 个片段"。
- [x] **工具调用带 clip 文件名**而非仅 `#id`："查看片段 A-0002.MP4 的台词" / "…的画面"（经 `_label()` 从 ClipDetail 取文件名，回落 `#id`）。
- [x] **导演思路**：agent loop 里模型工具调用之间的 reasoning 文本（`step.content`）截断后作为 "导演思路：…" 透出。
- [x] **收口/回落**："第 k 天改用快速生成…"（agent 不收口回落 staged）。
- [ ] （配合 28）critic 阶段："正在审片…" / "重做 2026-05-03（节奏拖沓）…"。

**前端：从"单行替换"改成"最近几步的滚动轨迹"**（`features/cutplan/index.tsx`）：
- [x] `progress: string` → `progressLog: string[]`；`pushProgress` 每个 tick append 最新串、跳过空串与相邻重复。
- [x] 聊天气泡渲染**最近 5 步**：最新一条带动画点，较旧的小字 + 灰色；空时回落 `roughcut.thinking`。分镜表横幅用最后一条。
- [x] 仍走轮询：每 tick 读到的新串 push 进列表，弥补 ~1.5s 轮询漏掉的中间步。

> 注意：进度串是临时态、轮询粒度 ~1.5s，比模型实际步进慢；"滚动轨迹"靠 append 累积可见历史。

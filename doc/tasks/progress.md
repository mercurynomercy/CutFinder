...
- [~] [**27 初剪实时进度 + 部分分镜先显示**](./27-rough-cut-live-progress.md) — **代码实现完成 + 自动测试通过（后端 526、前端 cutplan 6/6），真机验收待手动。** 逐天/逐片段实时状态（"第 k/N 天 · 查看片段 #X"）+ 已完成日期分镜先显示、其余标注生成中。沿用轮询（无 SSE/线程桥接）：director 把进度串(内存)+部分 plan 写进 store，前端 `resumePoll` 增量读取。**后续增强（待做）**：发更多更具体的进度事件（检索阶段、clip 标签/内容、导演思路文本、收口/回落）+ 前端从"单行替换"改成"最近几步滚动轨迹"，让用户看清导演思考到哪一步（见 27 文档「后续增强」节）。
- [ ] [**28 初剪 refine 按日期合并 + 审片 critic agent**](./28-rough-cut-refine-merge-and-critic.md) — 已设计、待执行。**①修 refine bug**：多轮"增加/重做某天"应**并入**已有分镜表而非整体替换（实测"增加一份 5/11"把整表替换成只剩 5/11；根因=确定性检索按改窄后的范围只重做该天 + plan 覆盖式保存）。方案：`generate(prior_plan=...)` 以旧 plan 为基底、按日期覆盖/保留、flatten 按日期排序。**②v2 审片 critic agent**：全片拼好后 critic 检查主观质量（节奏/叙事/A-B 配比）→ 点名日期复用①的"重做该日期并入"机制修一轮，默认关。两者共用同一套迭代机制故合并。把 25 的"每天一次纯 JSON 补全"升级为"每天一个 scoped 工具循环"（`get_clip_detail` 深挖 transcript + 按需 `inspect_broll` 看画面 + 自然收口 `emit_plan`），**确定性 Python 编排**（按天拆分/排序/时长均分/校验/失败兜底）骨架复用 25。实测：单日小上下文下 tool 收口可靠（Qwen3.6 5/5、Gemma4-26b 4/5），**不**用 named/required 强制收口（OMLX 不稳），靠 `tool_choice="auto"` + round-cap 兜底（不收口的天回落纯 JSON）。`cut_director_mode` 开关保留 staged 作 fallback。**v1 不上 critic 审片 agent**（v2）。设计见 `26-rough-cut-per-day-agent.md`。

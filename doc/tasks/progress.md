...
- [x] [**28 初剪 refine 按日期合并 + 审片 critic agent**](./28-rough-cut-refine-merge-and-critic.md) — **已实现 + 自动测试通过（后端 526 全绿、ruff/mypy 干净），真机验收待手动。** **①refine 按日期合并**：`generate(prior_plan=...)` 以旧 plan 为基底（按 chapter=拍摄日期建 `merged` 字典），本轮只重做范围内日期——成功覆盖该天、失败保留旧那天、`_flatten` 按日期排序后再 `_build_plan`；逐天 partial 与最终 plan 都走合并视图，故"增加一份 5/11"并入而非整体替换、refine 中途不闪掉旧日期；缩到无素材的日期返回 None plan（service 不覆盖、旧 plan 保留）。`cutplan_service.handle` 取 `get_latest_plan` 作 `prior_plan`。**②审片 critic（默认关）**：`cut_critic_enabled` 开关；全片拼好后一次 critic LLM 调用 → 结构化 `{date,issue,action}` 修订意见 → 复用①的"重做该日期并入"机制修一轮（坏 JSON/无意见则跳过、原 plan 不变）。后端 director +6 helper（`_gen_one_day`/`_flatten`/`_normalize_day`/`_shot_to_dict`/`_apply_critic`/`_critique`/`_plan_digest`）、+8 单测（refine 合并 ×2、prior_plan 透传 ×1、critic ×3 等）。**UI**：按用户决策，把「审片复检」+「视觉确认次数」两个**逐次生成参数**从全局设置页移到**初剪页面**——原「导演 Prompt」弹窗升级为「初剪设置」弹窗（顶部「生成选项」段放这两个控件，下面是导演 Prompt 文本框），开弹窗时 `GET /settings` 回填、保存时 `PUT /settings`（触发 director 重建生效）；全局设置页移除这两项。前端 cutplan +1 测试（206 全绿）。

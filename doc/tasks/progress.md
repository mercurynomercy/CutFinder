...
- [ ] [**28 初剪 refine 按日期合并 + 审片 critic agent**](./28-rough-cut-refine-merge-and-critic.md) — 已设计、待执行。**①修 refine bug**：多轮"增加/重做某天"应**并入**已有分镜表而非整体替换（实测"增加一份 5/11"把整表替换成只剩 5/11；根因=确定性检索按改窄后的范围只重做该天 + plan 覆盖式保存）。方案：`generate(prior_plan=...)` 以旧 plan 为基底、按日期覆盖/保留、flatten 按日期排序。**②v2 审片 critic agent**：全片拼好后 critic 检查主观质量（节奏/叙事/A-B 配比）→ 点名日期复用①的"重做该日期并入"机制修一轮，默认关。两者共用同一套迭代机制故合并。
- [ ] [**29 设置统一到 config.json，去掉 "env" 分组**](./29-settings-drop-env-grouping.md) — 已设计、待执行（纯清理）。machine-global 项（OMLX 端点/密钥、模型名）已全存 `~/.cutfinder/config.json`、无 OS env，但 settings API/前端仍用历史命名的 `"env"` 分组传递。本任务把 "env" 从 API/UI 去掉、统一成 config/prefs 视图。（本次已先修好相关脆测试：gallery `getByTitle`、settings mock 暂对齐现状。）把 25 的"每天一次纯 JSON 补全"升级为"每天一个 scoped 工具循环"（`get_clip_detail` 深挖 transcript + 按需 `inspect_broll` 看画面 + 自然收口 `emit_plan`），**确定性 Python 编排**（按天拆分/排序/时长均分/校验/失败兜底）骨架复用 25。实测：单日小上下文下 tool 收口可靠（Qwen3.6 5/5、Gemma4-26b 4/5），**不**用 named/required 强制收口（OMLX 不稳），靠 `tool_choice="auto"` + round-cap 兜底（不收口的天回落纯 JSON）。`cut_director_mode` 开关保留 staged 作 fallback。**v1 不上 critic 审片 agent**（v2）。设计见 `26-rough-cut-per-day-agent.md`。

...
- [ ] [**25 初剪导演 Agent（对话生成分镜表）**](./25-rough-cut-agent.md) — 已设计、待实现。**独立工具**：在已编目素材库之上多轮对话，按日期范围/时长/风格/节奏产出**精确到片段内 in/out 的文字分镜表（A-roll 叙事主线 + B-roll 插空，含缩略图 + 章节分组）**供照搬剪辑软件；**方案 C 受约束工具调用环**（确定性脚手架 + 护栏，LLM 只做创意选择），全本地走 Qwen3.6（OMLX），必要时 Qwen3-VL 现场看 B-roll；会话持久化 SQLite、可重开/可删除；**不渲染、不导出剪辑工程**（FCPXML 后续）。设计见 `25-rough-cut-agent.md` 与 `detailed-design.md` §3.15。

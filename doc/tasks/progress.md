...
- [~] [**27 初剪实时进度 + 部分分镜先显示**](./27-rough-cut-live-progress.md) — **代码实现完成 + 自动测试通过（后端 526、前端 cutplan 6/6），真机验收待手动。** 逐天/逐片段实时状态（"第 k/N 天 · 查看片段 #X"）+ 已完成日期分镜先显示、其余标注生成中。沿用轮询（无 SSE/线程桥接）：director 把进度串(内存)+部分 plan 写进 store，前端 `resumePoll` 增量读取。**更丰富进度（已做大部分）**：后端发检索阶段/本天片段数/clip 文件名/导演思路/回落等更具体事件；前端 `progressLog` 把"单行替换"改成"最近 5 步滚动轨迹"（最新带动画点）。仅剩 critic 阶段串待 28。
- [x] [**28 初剪 refine 按日期合并 + 审片 critic agent**](./28-rough-cut-refine-merge-and-critic.md) — **已实现 + 自动测试通过（后端 526 全绿、ruff/mypy 干净），真机验收待手动。** **①refine 按日期合并**：`generate(prior_plan=...)` 以旧 plan 为基底（按 chapter=拍摄日期建 `merged` 字典），本轮只重做范围内日期——成功覆盖该天、失败保留旧那天、`_flatten` 按日期排序后再 `_build_plan`；逐天 partial 与最终 plan 都走合并视图，故"增加一份 5/11"并入而非整体替换、refine 中途不闪掉旧日期；缩到无素材的日期返回 None plan（service 不覆盖、旧 plan 保留）。`cutplan_service.handle` 取 `get_latest_plan` 作 `prior_plan`。**②审片 critic（默认关）**：`cut_critic_enabled` 开关；全片拼好后一次 critic LLM 调用 → 结构化 `{date,issue,action}` 修订意见 → 复用①的"重做该日期并入"机制修一轮（坏 JSON/无意见则跳过、原 plan 不变）。后端 director +6 helper（`_gen_one_day`/`_flatten`/`_normalize_day`/`_shot_to_dict`/`_apply_critic`/`_critique`/`_plan_digest`）、+8 单测（refine 合并 ×2、prior_plan 透传 ×1、critic ×3 等）。**UI**：按用户决策，把**逐次生成参数**从全局设置页移到**初剪页面**——原「导演 Prompt」弹窗升级为「初剪设置」弹窗（顶部「生成选项」段 + 下面导演 Prompt 文本框）。生成选项含「生成模式」`cut_director_mode`(agent/staged)、「最大工具轮数」`cut_max_tool_rounds`(仅 agent 显示)、「审片复检」`cut_critic_enabled`、「视觉确认次数」`cut_vision_budget`；开弹窗 `GET /settings` 回填、保存 `PUT /settings`（触发 director 重建生效）；全局设置页移除这些项。前端 cutplan +1 测试（206 全绿）。
- [x] [**29 设置统一到 config.json，去掉 "env" 分组**](./29-settings-drop-env-grouping.md) — **已实现（纯清理）。** 采用方案 B：`GET /api/settings` 把 machine-global 键（`OMLX_BASE_URL`/`OMLX_API_KEY`(masked)/`TEXT_MODEL`/`VISION_MODEL`）并进 `prefs` 视图、删掉 `env` 分组（`settings_routes.py`；PUT 写入路径不变，仍按 `_GLOBAL_KEYS` 落 config.json）。前端 `settings/index.tsx` 从 `data.prefs.*` 读、`client.ts` 的 `SettingsResponse` 去掉 `env`、`SettingsPrefs` 补 4 个可选 global 键；mock `handlers.ts` 同步并入 prefs；`schemas.SettingsOut` 去 `env`。后端 `test_app_factory` 契约改断言（`"env" not in resp`、masked 在 prefs）。后端 526 全绿、前端 205 全绿，ruff/mypy/tsc 干净（仅余 tsconfig baseUrl 历史告警）。`config.py` 内部命名（`EnvSettings`/`_GLOBAL_KEYS`）按设计留作后续，仅改对外 API/UI。
- [x] [**30 初剪 fallback 复用 agent 已勘察分析**](./30-rough-cut-fallback-analysis-reuse.md) — **已实现 + 自动测试通过（后端 538、前端 206），真机 eval 待手动。** 单天 mini-agent 收不了口回落到快速模式（`_staged_day`）时，把 agent 已**花视觉预算**的 `inspect_broll` 画面描述带进快速模式 prompt（`_run_day` 累积 `findings` → `_gen_one_day` 透传 → `_staged_day` 注入「导演已现场勘察…」段；`vision_used` 自增作成功信号、错误串不记）。回落进度行显示「带入 N 条已勘察画面」。超/欠时长**不裁**、沿用既有 `within_target` ⚠️ flag + 补一句"素材有限"文案。设计见 `30-rough-cut-fallback-analysis-reuse.md` 与 `docs/superpowers/specs/2026-06-24-rough-cut-fallback-analysis-reuse-design.md`。

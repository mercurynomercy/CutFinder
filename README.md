...
> CutFinder runs **only on M-series Macs**. Every model runs locally — the text/vision models via a local **OpenAI-compatible inference server** (OMLX is the reference implementation this project was built against, MLX-based; LM Studio, Ollama's OpenAI-compat endpoint, and vLLM also work) and `mlx-whisper` / Qwen3-ASR + ForcedAligner (speech, via Apple's MLX framework) are all **Apple-Silicon-only**. Intel Macs, Windows, and Linux are **not supported**.
...
> **One prerequisite lives outside the app:** a local **OpenAI-compatible model server** — [OMLX](https://github.com/jundot/omlx) is the reference implementation this project was built against (LM Studio, Ollama's OpenAI-compat endpoint, and vLLM also work). Install one and load `Qwen3.6-35B-A3B` (text) + `Qwen3-VL-8B` (vision). CutFinder detects it on first run and **guides you if it's missing** — scanning / transcription / thumbnails still work without it; only A-roll summaries and B-roll tags need it.
...
1. **Configure the AI service** — **Settings → OpenAI-compatible connection** → fill in Base URL / API key → Save. Stored in `~/.cutfinder/config.json` (machine-wide; the Whisper model and the vocal-separation / auto-keyframe toggles are stored there too).
...
- **Launcher home screen** — opening CutFinder lands on a card-grid entry screen (Gallery / Settings / Tasks / Rough-cut / Subtitles); the logo returns here from any full-screen page.
...
- **Full-screen clip detail** — clicking a thumbnail opens a full-screen detail view (not an overlay/drawer); the AI summary/description is read-only, tags stay editable (add/delete), and corrections are remembered.
...
- **Rough-cut director — conversational shot list (beta)** — describe the cut you want in chat (date range, target length, aspect ratio, style/rhythm) and a local Qwen text model drafts a precise, in/out-level **shot list** from your cataloged footage: chaptered **by shooting date**, ordered along the real shooting timeline within each day, with an A-roll narration spine (transcript-driven) plus B-roll cutaways. The page is a **three-column layout**: a session sidebar on the left, the chat thread in the middle, and the live/finished date-chaptered shot panel on the right. Each row shows the clip's date and file so you can find the source fast; copy the whole plan as Markdown into your editor. Scoping (dates / length / aspect) is parsed straight from your message, and generation runs **a scoped tool loop per shooting date** (the model deep-dives transcripts and peeks at B-roll frames only as needed) for reliability on local models, showing **live progress and rendering finished dates while the rest generate** (the finished progress trace collapses to a one-line "generation steps" summary you can expand). **If your request is ambiguous** — no date range across a multi-date library, or no target duration — the director asks a quick clarifying question as clickable chips instead of guessing; picking one resumes generation exactly where that day's tool loop paused. Follow-up edits like "add a day" or "redo 5/3" **merge into** the existing shot list instead of replacing it, and an optional **critic review pass** (off by default) re-checks subjective quality and redoes the dates it flags. The **director prompt** and these generation options live in a dedicated "Rough-cut settings" page. Read-only over the catalog — it never renders or exports an edit project. **Still in beta and actively being improved.**
...
- **Task queue management** — a dedicated Task Queue page lists every scan / re-analyze job, with delete, retry-failed, and global pause/resume; scanning prompts you when the queue is paused. Job progress (scans, keyframes) and subtitle-export progress both show in a **fixed bottom status bar**.
...
- **Settings → Maintenance** — a dedicated section holds housekeeping: **"Clean up deleted files"** removes orphaned catalog entries (plus thumbnails/keyframes) for copies you deleted straight from the library folder (skipped, not wiped, if the library is unreachable — e.g. an external drive unmounted), and a link to the full-screen **Logs** page. Originals are never touched. (Relocated from the old header overflow menu — the top bar is now nav links + global search only.)
...
Adapters ── ffmpeg/ffprobe · Silero VAD · mlx-whisper / Qwen3-ASR+ForcedAligner · OpenAI-compatible server (text + vision) · Pillow (photos) · SQLite
```
...
| Purpose | Model | How it runs |
|---|---|---|
| A-roll summary/tags (text) | `Qwen3.6-35B-A3B` | Local OpenAI-compatible server (`/chat/completions`) |
| B-roll visual recognition (vision) | `Qwen3-VL-8B` | Same server, frames sent as base64 |
| Photo description + tags (vision) | `Qwen3-VL-8B` | Same server, a JPEG preview of the photo sent as base64 |
| A-roll speech transcription | **Speech engine, selectable in Settings:** `mlx-whisper` (default `mlx-community/whisper-large-v3-mlx`) **or** Qwen3-ASR + ForcedAligner (`mlx-community/Qwen3-ASR-1.7B-8bit` + `Qwen3-ForcedAligner-0.6B-8bit`) | Separate local process (the server does not serve audio) |
...
Text and vision models are **both served by a local OpenAI-compatible server** — any server that exposes the standard `/chat/completions` API works. [OMLX](https://github.com/jundot/omlx) is the reference implementation this project was built against (Apple-Silicon inference server, menu-bar app); LM Studio, Ollama's OpenAI-compat endpoint, and vLLM are also fine. Models are switched via the `model` name on the same endpoint — the server handles load/unload and memory (LRU eviction, model pinning, per-model TTL) so both models can coexist.
...
> ⚠️ The model names must match exactly the ids your server has loaded. The default vision model is `Qwen3-VL-8B`; if your server exposes a suffixed id, change `vision_model` / `text_model` in Settings or in `<library>/.cutfinder/config.json`.
...
| A local **OpenAI-compatible model server** | Exposes `/chat/completions`; [OMLX](https://github.com/jundot/omlx) ≥ 0.1 is the reference implementation (Apple-Silicon menu-bar app), LM Studio / Ollama / vLLM also work — preload `Qwen3.6-35B-A3B` (text) and `Qwen3-VL-8B` (vision) |
...
- [Homebrew](https://brew.sh/) — to install ffmpeg / OMLX (or your chosen OpenAI-compatible server)
...
> The OpenAI-compatible server connection is configured in the **Settings page** (step 2) — no config file to edit by hand.
...
### 2. Configure the OpenAI-compatible connection

Configure two things — the server URL and API key:

- **Settings page (recommended)** — after launching, open http://localhost:5080 → **Settings** → **OpenAI-compatible connection** → fill in Base URL / API key → Save. These are stored in `~/.cutfinder/config.json` (**shared machine-wide**, no need to re-enter per library) and take effect immediately.

- **OS environment variables (optional)** — export `OPENAI_BASE_URL` / `OPENAI_API_KEY` in your shell (handy for CI / one-off runs):
...
```bash
make check-openai               # checks that the text/vision models are loaded
# → OpenAI-compatible server OK — models: [...]
#   All required text/vision models are present.
```

> `make check-openai` resolves creds the same way the app does (`~/.cutfinder/config.json` > OS env vars), so it works whether you configured via the UI or via env vars.
...
```bash
# Terminal 1 — backend (OpenAI-compatible server config comes from ~/.cutfinder/config.json or exported env vars)
cd backend
...
uv run pytest -m integration         # integration tests (need a real OpenAI-compatible server / ffmpeg / sample clips)
uv run mypy cutfinder/               # type check (strict, clean)
uv run ruff check cutfinder/         # linting (clean)
```

Integration tests **auto-skip** when the OpenAI-compatible server / sample clips are missing — no false failures. To actually exercise the AI chain, configure the server connection (in the Settings UI or via env vars), then:
...
make test              # full backend (incl. -m integration; runs for real when the OpenAI-compatible server is configured, may be slow)
make test-integration  # only -m integration (needs ffmpeg + a configured OpenAI-compatible server)

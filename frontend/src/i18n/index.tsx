...
const en = {
  ...
  'roughcut.leanBudget': 'Per-day catalog size (agent, chars)',
  'roughcut.leanBudgetDesc': 'How much of a day’s clip catalog the agent sees (one short line per clip; transcripts fetched on demand). Higher = no clips dropped on busy days; the text model takes a huge context, so the cap is just to bound local prefill cost.',
  'roughcut.stagedBudget': 'Per-day catalog size (fast, chars)',
  'roughcut.stagedBudgetDesc': 'Fast mode inlines transcripts (it has no tools), so it fills faster than agent mode. Higher = fuller transcripts reach the model, at more local prefill cost.',
...
const zh: Record<Key, string> = {
  ...
  'roughcut.leanBudget': '单日素材目录上限（Agent，字符）',
  'roughcut.leanBudgetDesc': 'Agent 模式下这一天能看到的素材目录大小（每片段一行摘要，台词按需用工具取）。越大越不会在素材多的日子丢片段；文本模型可吃超大上下文，这个上限只是为了约束本地推理开销。',
  'roughcut.stagedBudget': '单日素材目录上限（快速，字符）',
  'roughcut.stagedBudgetDesc': '快速模式会把台词内联进目录（它没有工具可取），所以比 Agent 模式填得更快。越大送进模型的台词越完整，本地推理开销也越高。',

...
- **界面语言可选（中 / 英）**：整套 UI 文案可在「设置」页切换**英文 / 中文**（默认英文，按设备记忆），与下面的 **AI 输出语言**相互独立、互不影响。该选择现在也驱动**初剪导演的自带 Prompt 与实时进度文案**（均备有中英两版）。
...
- **深色专业界面**：近黑面板让缩略图突出，A-roll/B-roll 以颜色+图标区分，贴近 FCP 调性（见 [`docs/detailed-design.md`](./docs/detailed-design.md) §UI 设计系统）。
...
- [UI 设计系统 `docs/detailed-design.md`](./docs/detailed-design.md) (§ UI 设计系统部分) —— 配色/字体/间距 token、组件规范、页面布局（深色优先）
- [任务清单 `docs/tasks/README.md`](./docs/tasks/README.md) —— 各模块最小任务与总体进度

"""Verify the OpenAI-compatible server is reachable and required models are loaded.

Reads OPENAI_BASE_URL / OPENAI_API_KEY via CutFinder's config layer
...
def _resolve_openai_config() -> Tuple[str, str]:
    """Return ``(base_url, api_key)`` using the same precedence as the app.

    Priority: ``~/.cutfinder/config.json`` > OS env var.
    Falls back to a common local default URL if no base_url is set anywhere.

    Extracted as a separate function for testability — tests can patch
    ``cutfinder.config.resolve_env`` without needing a real server.
    """

    env = resolve_env()
    base_url = env.OPENAI_BASE_URL or "http://localhost:8000/v1"
...
def main() -> int:
    base, key = _resolve_openai_config()

    if not key:
        print("OPENAI_API_KEY is empty — set it in the Settings UI or as an env var", file=sys.stderr)
        return 1
    ...
    try:
        ...
        print(f"OpenAI-compatible server unreachable at {base}: {exc}", file=sys.stderr)
        return 1

    models = [m["id"] for m in resp.json().get("data", [])]
    print("OpenAI-compatible server OK — models:", models)
    ...
    if missing:
        print("Missing required models: " + ", ".join(missing), file=sys.stderr)
        return 1
